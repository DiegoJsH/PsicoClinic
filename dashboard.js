...
async function cargarTotalPacientes() {
  try {
    ...
    agregarAlerta(
      "info",
      `Hay ${pacientes.length} pacientes registrados actualmente.`
    );
...
function mostrarProximasCitas(citas) {
  ...
  if (citas.length === 0) {
    contenedor.innerHTML =
      '<p class="text-center text-muted">No hay citas programadas</p>';
    return;
  }
...
function agregarAlerta(tipo, mensaje) {
  ...
  alerta.innerHTML = `<small><strong>${
    tipo === "warning" ? "Aviso:" : "Info:"
  }</strong> ${mensaje}</small>`;
